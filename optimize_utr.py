...
def optimize_sequence(utr, cds, target_tir, iterations=1000):
    s = list(utr)
    s_old = s[:]
    nuc = ["A", "G", "C", "T"]
    utr_length = len(utr)

    I = I_old = round(calculateInitiationRate(utr, cds), 4)
    kbt_values = [1e-01]*1 + [1e-02]*1 + [1e-03]*3 + [1e-04]*3 + [1e-05]*2
    index = 0
    ...
    for i in range(iterations):
        ...
        r1 = random.randint(0, utr_length - 1)
        ...
        I_new = round(calculateInitiationRate(utr_new, cds), 4)
        E1 = abs(target_tir - I)
        E2 = abs(target_tir - I_new)
        delE = E2 - E1
    ...
    optimized_utr = ''.join(s_old)
    return {
        "optimized_utr": optimized_utr,
        "predicted_initiation_rate": round(I, 4)
    }
